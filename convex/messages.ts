...
import { ensureEnvVar } from "./convexProjects";
...
export const setUrlId = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing.urlId === undefined) {
      const urlId = await _allocateUrlId(ctx, { urlHint });
...
async function _allocateUrlId(ctx: QueryCtx, { urlHint }: { urlHint: string }) {
  const existing = await getChatByUrlId(ctx, { id: urlHint });
  ...
  while (true) {
    ...
    const m = await getChatByUrlId(ctx, { id: newUrlId });
...
function getChatByInitialId(ctx: QueryCtx, { id }: { id: string }) {
  return ctx.db
    .query("chats")
    .withIndex("byInitialId", (q) => q.eq("initialId", id))
    .filter((q) => q.neq(q.field("isDeleted"), true))
    .unique();
}

function getChatByUrlId(ctx: QueryCtx, { id }: { id: string }) {
  return ctx.db
    .query("chats")
...
export async function getChatByIdOrUrlIdEnsuringAccess(
  ...
  const byId = await getChatByInitialId(ctx, { id });
  if (byId !== null) {
    return byId;
  }
